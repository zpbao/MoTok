...
from models.vqvae import VectorQuantizedVAE
from models.perceiver import PerceiverDecoder
...
class SlotAttention(nn.Module):
    def __init__(self, num_slots, dim, eps = 1e-8, hidden_dim = 128):
        ...
        self.norm1_predictor = nn.LayerNorm(dim)
        self.norm2_predictor = nn.LayerNorm(dim)
    ...
    def forward(self, inputs, num_slots = None):
        ...
        for f in range(nf):
            ...
            cur_slots = self.norm1_predictor(cur_slots + self.self_attn(cur_slots,cur_slots,cur_slots)[0])
            cur_slots = self.norm2_predictor(self.fc3(cur_slots) + cur_slots)
            slots = cur_slots
...
class Encoder(nn.Module):
    def __init__(self, resolution, hid_dim, input_channel):
        super().__init__()
...
class SlotAttentionAutoEncoder(nn.Module):
    def __init__(self, resolution, num_slots, hid_dim, output_channel, num_token, depth = 2):
        ...
        self.encoder_cnn = Encoder(self.resolution, self.hid_dim, input_channel = self.output_channel)
        self.vqvae = VectorQuantizedVAE(self.output_channel, self.hid_dim, num_token)
        ...
        self.slot_attention = SlotAttention(
            ...
            eps = 1e-8, 
            hidden_dim = 128)
        self.LN = nn.LayerNorm([self.dresolution[0] * self.dresolution[1], hid_dim])
        self.slot2vq = PerceiverDecoder(depth = depth, queries_dim = self.hid_dim, logits_dim = self.hid_dim, latent_dim = self.hid_dim)
        self.positional_encoder = PositionalEncoding(self.dresolution[0]*self.dresolution[1], self.hid_dim, 0.1)
    ...
    def forward(self, image):
        ...
        attn_masks = attn_masks.view(bs,n_frames, self.num_slots, self.dresolution[0], self.dresolution[1])

        x1 = torch.zeros_like(x)
        x1 = x1.view(bs*n_frames,-1,self.hid_dim)
        x1 = self.positional_encoder(x1)
        s1 = slots_ori.reshape(bs*n_frames,self.num_slots,self.hid_dim)

        # print(x1.shape, s1.shape)
        slots_combine = self.slot2vq(s1,x1) # b,n,d
        # print(slots_combine.shape)

        slots_combine = slots_combine.permute(0,2,1).view(bs*n_frames, self.hid_dim,self.dresolution[0],self.dresolution[1])

        # `slots_combine` has shape: [batch_size*num_slots, width_init, height_init, slot_size].
        recons, z_e_x, z_q_x, latents = self.vqvae(image.view(bs*n_frames, dim, him, wim))
        # `recons` has shape: [bs, n_frames, 3, H//4, W // 4]

        return recons, attn_masks, slots_combine, z_e_x, z_q_x, latents
