...
from dataset.datasetKITTI import KITTIDataset
...
parser.add_argument('--weight_token', default = 1.0, type = float, help = 'weight for the token loss')
...
def main():
    ...
    train_set = KITTIDataset(split = 'train', root = data_path)
    test_set = KITTIDataset(split = 'eval', root = data_path)

    model = SlotAttentionAutoEncoder(resolution, opt.num_slots, opt.hid_dim, 3, opt.num_tokens).to(device)
    model = nn.DataParallel(model)
    state_dict = torch.load('./pre-trained/pd-est.ckpt')['model_state_dict']
