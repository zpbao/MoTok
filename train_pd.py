...
from dataset.datasetPD import PDDataset
...
parser.add_argument('--weight_token', default = 0.05, type = float, help = 'weight for the token contrastive loss')
...
def main():
    ...
    train_set = PDDataset(split = 'train', root = data_path)
    test_set = PDDataset(split = 'eval', root = data_path)
    ...
    optimizer = optim.AdamW(params, lr=opt.learning_rate, weight_decay = 0.05)
    ...
    for epoch in range(opt.num_epochs):
        ...
        for sample in tqdm(train_dataloader):
            ...
            moving_masks = F.interpolate(moving_masks.float(), (120, 242)).long().to(device)
            ...
            if opt.supervision == 'est':
                for i in range(opt.batch_size):
                    for j in range(5):
                        matches = scipy.optimize.linear_sum_assignment(-scores[i,j])
                        id_slot, id_gt = matches 
                        loss_mask += bcecriterion(masks[i,j,id_slot,:,:], moving_masks[i,j,id_gt,:,:])
            else:
            ...
            whole_loss = loss + opt.weight_vq * vq_loss + opt.weight_mask * loss_mask + opt.weight_token * t_loss
            ...
            del recon_combined, masks, image, loss, whole_loss, slots, z_q_x, latents, vq_loss, z_e_x, moving_masks, loss_mask
            # break
        ...
        if opt.wandb:
            wandb.log({"recon_loss": total_loss,  "vq_loss": VQ_loss, 'mask_loss': M_loss})
