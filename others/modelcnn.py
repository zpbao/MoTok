...
class SlotAttentionAutoEncoder(nn.Module):
    def __init__(self, resolution, num_slots, hid_dim, output_channel, num_token):
        ...
        self.conv1 = nn.Conv2d(self.hid_dim, self.hid_dim, kernel_size = 3)
        self.conv2 = nn.Conv2d(self.hid_dim, self.hid_dim, kernel_size = 3)

        self.decoder = Decoder(self.hid_dim, self.resolution, 2)
    ...
    def forward(self, image):
        ...
        # slots = slots.repeat((1, 1, self.dresolution[0], self.dresolution[1], 1))
        slots_combine = slots * attn_masks
        slots_combine = slots_combine.sum(dim = 1)
        slots_combine = slots_combine.permute(0,3,1,2)
        slots_combine = self.conv1(slots_combine)
        slots_combine = F.relu(slots_combine)
        slots_combine = self.conv2(slots_combine)
