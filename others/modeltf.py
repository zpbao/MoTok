...
from models.vqvae import VectorQuantizedVAEDecoder as vqdecoder
...
class SlotAttentionAutoEncoder(nn.Module):
    def __init__(self, resolution, num_slots, hid_dim, output_channel, num_token):
        ...
        # self.vqdecoder = vqdecoder(self.output_channel, self.hid_dim, num_token)
        ...
        self.slot2vq = TransformerDecoder(
            1, self.dresolution[0]*self.dresolution[1], self.hid_dim, 8, 0.1)
        self.out = linear(self.hid_dim, 64, bias=False)
    ...
    def forward(self, image):
        ...
        slots_combine = self.slot2vq(x1,s1) # b,n,d
        slots_combine = self.out(slots_combine)
        ...
        return recons, attn_masks.view(bs,n_frames, self.num_slots, attn_masks.shape[2], attn_masks.shape[3]),slots_combine, z_e_x, z_q_x, latents
